...
def generator_x(input_z, reuse=False):
    ...
    with tf.variable_scope('generator_x', reuse=reuse):
        ...
        with tf.variable_scope('block5'):
            net = leaky_relu(bn(conv2d(net, fmaps=64, kernel=3, use_wscale=False), name='b_4')) #128*128
            net = upscale2d(net, factor=2)

        with tf.variable_scope('output'):
            net = apply_bias(conv2d(net, fmaps=3, kernel=3, use_wscale=False))  # 128*128
...
def generator_z(input_x, reuse=False):
    ...
    with tf.variable_scope('generator_z', reuse=reuse):
        ...
        with tf.variable_scope('block0'):
            net = leaky_relu(bn(conv2d(input_x, fmaps=64, kernel=7, use_wscale=False), name='b_1')) #64*64
            net = downscale2d(net, factor=2)

        with tf.variable_scope('block1'):
            net = leaky_relu(bn(conv2d(net, fmaps=64, kernel=5, use_wscale=False), name='b_1')) #32*32
            net = downscale2d(net, factor=2)
...
def discriminator(input_x, input_z, reuse=False):
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        with tf.variable_scope('input_x'):
            ...
            with tf.variable_scope('block1'):
                net = leaky_relu(conv2d(input_x, fmaps=64, kernel=7, use_wscale=False)) #128*128
                net = tf.nn.dropout(net, keep_prob=0.8)

            with tf.variable_scope('block2'):
                net = conv2d(net, fmaps=64, kernel=5, use_wscale=False)         # 64*64
                net = downscale2d(net, factor=2)
                net = tf.nn.dropout(leaky_relu(bn(net, name='b_1')), keep_prob=0.8)

            with tf.variable_scope('block3'):
                net = conv2d(net, fmaps=128, kernel=3, use_wscale=False)         # 32*32
                net = downscale2d(net, factor=2)
                net = tf.nn.dropout(leaky_relu(bn(net, name='b_1')), keep_prob=0.8)

            with tf.variable_scope('block4'):
                net = conv2d(net, fmaps=256, kernel=3, use_wscale=False)         # 16*16
                net = downscale2d(net, factor=2)
                net = tf.nn.dropout(leaky_relu(bn(net, name='b_2')), keep_prob=0.8)

            with tf.variable_scope('block5'):
                net = conv2d(net, fmaps=256, kernel=3, use_wscale=False)         #8*8
                net = downscale2d(net, factor=2)
                net = tf.nn.dropout(leaky_relu(bn(net, name='b_3')), keep_prob=0.8)

            with tf.variable_scope('block6'):
                net = conv2d(net, fmaps=512, kernel=3, use_wscale=False)         #4*4
                net = downscale2d(net, factor=2)
